...
// ── 站内搜索的手写文本 ──
//
// 这里原本还抄了一份 name（产品与案例）和 tag（案例）。name 与事实源当前一致，
// 但手抄迟早会漂移；caseSummaries.tag 则从未被任何代码读取过，是死数据。
// 现在 name 一律从事实源取，tag 删掉，只保留真正需要人工撰写的部分：
// 去标点、补同义词的搜索全文（产品页写「拾音半径 5m」，这里写「5米拾音半径」）。
//
// 新增产品或案例时必须在这里补一条搜索文本，否则它会静默地从站内搜索里消失 ——
// doctor 会检查覆盖率，漏了会报错。
interface ProductSearchText {
  description: string;
  /** 搜索用全文。缺省时回退到 description（a18 就是这种情况） */
  fullText?: string;
}

const PRODUCT_SEARCH_TEXT: Record<string, ProductSearchText> = {
  a1: {
    description: "MC10是思必驰推出的一款高端吸顶会议麦克风，其集成了多达128单元的全向麦克风阵列，提供了16个独立可配拾音区，可实现精细化拾音配置。采用了Dante数字音频技术, 确保音频传输稳定和广泛兼容。MC10内置AI算法，提供高清降噪、去混响、反馈抑制、自动增益和语音转写等功能。",
  ...
  a2: {
    description: "无感扩声新标杆，3m 拾音半径，>18dB 增益，AI 降噪 + 反馈抑制双算法。48kHz 采样率，低于 15ms 延迟。",
  ...
  a3: {
    description: "32 单元全向阵列，4 个拾音区，支持 Dante，集拾音扩声于一体。PoE+ 供电。",
  ...
  a4: {
    description: "4K 超高清，12 倍光学 + 16 倍数字变焦，适配各类企业会议室。支持 HDMI/SDI/USB 输出。",
  ...
  a5: {
    description: "音视频融合追踪引擎，多种追踪模式（声源追踪、人脸追踪、区域追踪），适配企业与教育演讲场景。",
  ...
  a6: {
    description: "全频同轴天花扬声器，PoE 供电，适用商店、会议室、酒店多场景。Dante 数字音频。",
  ...
  a7: {
    description: "32 单元阵列，8 个独立配置拾音区，专为教学场景精心设计。支持 Dante，AI 降噪。",
  ...
  a8: {
    description: "集拾音、扩音、语音转写、字幕同传于一体，多台级联，覆盖大中小型会议室。",
  ...
  a9: {
    description: "多种 AI 追踪模式，实时字幕，音视频融合，适配会议讨论、演讲、板书。",
  ...
  a10: {
    description: "智能控制 + 精准拾音 + 便捷部署，现代高效会议的得力助手。触控操作，PoE 供电。",
  ...
  a11: {
    description: "24单元MEMS阵列，2m精准扩声覆盖，ClearSpeakAI算法，专为教室教学打造。",
  ...
  a12: {
    description: "专为MA600D矩阵麦克风桌面部署定制，安装更美观整洁，适配高端会议空间。",
  ...
  a19: {
    description: "集无线投屏、BYOM会议、HDMI矩阵切换、会议室中控于一体，4K@60Hz超清画质，双网物理隔离，USB-C一线通60W供电，支持四画面同屏。",
...
interface CaseSearchText {
  snippet: string;
  fullText?: string;
}

const CASE_SEARCH_TEXT: Record<string, CaseSearchText> = {
  e1: {
    snippet: "教室空间较大，面向国际对实时中英翻译有需求。采用吸顶麦MC10+C60摄像头声像追踪方案，实现全向拾音和实时翻译投屏。",
  ...
  e2: {
    snippet: "教室隔音不好，现有扩声设备效果差。采用MC10+DC20，有效降噪拾音，实现无感扩声。",
  ...
  e3: {
    snippet: "教室面积大，手持麦频繁啸叫。采用MC10+DP88，实现高音质本地扩声，降本增效。",
  ...
  e4: {
    snippet: "培训教室希望改造成无感扩声教室。采用MC08+利旧设备，设置拾音屏蔽区，过滤走步声板书声。",
  ...
  e5: {
    snippet: "大阶梯教室有啸叫和混响。采用MC10吸顶麦，覆盖讲台和学生区域，教师彻底告别手持麦。",
  ...
  e6: {
    snippet: "120平米会见室传统扩声设备繁多。采用2片MC10+7台DC20级联，实现大空间轻松扩音。",
  ...
  e7: {
    snippet: "传统舞台圆桌论坛麦克风传递打乱节奏。采用3台矩阵麦，智能拾音区定制，15ms超低延迟无感扩声。",
  ...
  e8: {
    snippet: "每日密集会议排期，手持麦反复充电调试。采用MC10吸顶麦一次部署无需重复调试，翻台效率大幅提升。",
  ...
  e9: {
    snippet: "会议室面积大，原有拾音设备不理想。采用MC10+DP88，实现大空间无感扩声，吸顶设计美观便利。",
  ...
  e10: {
    snippet: "传统视频会诊系统单台摄像机角度受限。采用MC10+C60声像追踪方案，实现全场景智能拾音和多角度发言人追踪。",
  ...
  e11: {
    snippet: "跨部门跨地区沟通协作需求高。采用MC10+MT100+C40T声像追踪方案，会议效率提升30%。",
  ...
  e12: {
    snippet: "300平展厅对接语音数字人，需屏蔽周边噪音。采用4台MC10+DP88，构建全域高质量语音覆盖。",
...
/**
 * 搜索条目 = 事实源的 name + 上面手写的搜索文本。
 *
 * 用 filter + map 而非直接 map：漏写搜索文本的条目会被排除，
 * 而 doctor 会报出这个缺口 —— 两个信号互相印证，不会静默少一条。
 *
 * 按 id 数字排序，而不是沿用事实源的展示顺序：search() 只按 score 排序
 * （Array.sort 是稳定排序），同分结果的先后完全由这里的条目顺序决定 ——
 * 顺序就是行为。products.json 的展示顺序是有人调过的（a2 排第一），
 * 如果直接沿用，搜「麦克风」这类同分关键词的结果顺序就会变。
 */
const byNumericId = (a: { id: string }, b: { id: string }) =>
  Number(a.id.slice(1)) - Number(b.id.slice(1));

const productDetails = PRODUCTS
  .filter((p) => PRODUCT_SEARCH_TEXT[p.id])
  .map((p) => {
    const t = PRODUCT_SEARCH_TEXT[p.id];
    return { id: p.id, name: p.name, description: t.description, fullText: t.fullText };
  })
  .sort(byNumericId);

const caseSummaries = CASES
  .filter((c) => CASE_SEARCH_TEXT[c.id])
  .map((c) => {
    const t = CASE_SEARCH_TEXT[c.id];
    return { id: c.id, name: c.name, snippet: t.snippet, fullText: t.fullText };
  })
  .sort(byNumericId);
...
function highlightText(text: string, query: string): React.ReactNode {
  ...
  return parts.map((part, i) =>
    regex.test(part) ? (
      <mark key={i} style={{ background: 'rgba(204,120,92,0.18)', color: 'var(--warm-ink)', borderRadius: 2, padding: '0 1px' }}>{part}</mark>
    ) : (
      part
    )
  );
...
function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  useEffect(() => {
    ...
    // 归还焦点：否则键盘用户关掉弹窗后会掉回页面顶部
    previouslyFocused.current?.focus?.();
  }, [isOpen]);

  /**
   * 焦点陷阱。
   * 弹窗对读屏与键盘用户此前是「一个没有身份的浮层」——Tab 会跑到背后的页面上。
   */
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="hd-search-title"
      ...
      >
        ...
        {/* 搜索输入行 */}
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px 20px', borderBottom: results.length > 0 ? '1px solid var(--warm-hairline)' : 'none' }}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" style={{ flexShrink: 0, marginRight: 12, color: 'var(--warm-muted)' }}>
          ...
          <input
            ...
            aria-controls="hd-search-listbox"
            aria-autocomplete="list"
            aria-activedescendant={results.length > 0 ? `hd-search-opt-${activeIdx}` : undefined}
            aria-label="搜索产品、案例与常见问题"
            style={{
              flex: 1, border: 'none', outline: 'none',
              fontSize: 16, color: 'var(--warm-ink)',
          ...
          <button
            ...
            style={{
              ...
              fontSize: 18, color: 'var(--warm-muted)',
              background: 'transparent', border: 'none',
              borderRadius: 6, cursor: 'pointer',
              fontFamily: 'inherit', lineHeight: 1,
              transition: 'background 0.15s, color 0.15s',
            }}
            onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--warm-surface)'; e.currentTarget.style.color = 'var(--warm-ink)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--warm-muted)'; }}
          >
            ×
          </button>
        </div>
        ...
        >
          {results.length > 0 ? (
            results.map((item, idx) => (
              <div
                ...
                style={{
                  ...
                  borderBottom: idx < results.length - 1 ? '1px solid var(--warm-hairline)' : 'none',
                  transition: 'background 0.1s',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--warm-card-hover)'; }}
              ...
              >
                ...
                <span style={{
                  ...
                  color: item.type === 'product' ? 'var(--warm-primary)' : item.type === 'case' ? 'var(--warm-muted)' : 'var(--warm-on-dark-soft)',
                ...
                <div style={{ overflow: 'hidden', flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--warm-ink)', marginBottom: 3, lineHeight: 1.4 }}>
                    {highlightText(item.title, query)}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--warm-muted)', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
            ...
            <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--warm-muted)', fontSize: 14 }}>
              未找到相关内容
            </div>
          ) : (
            <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--warm-on-dark-soft)', fontSize: 13 }}>
              输入关键词搜索产品、案例和常见问题
            </div>
          )}
        </div>
...
export default function Navigation() {
  ...
  const mobileNavItems = [
    ...
    { label: '选型助手', action: () => scrollTo('picker') },
    ...
    <>
      <motion.nav
        ...
        style={{
          ...
          borderBottom: '1px solid var(--warm-hairline)',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '0 24px',
        }}
      >
        {/* 左：Logo
            原来是带 onClick 的 <span> —— 键盘用户根本聚焦不到它。
            改成真按钮，可访问名来自其中的文字。 */}
        <button
          type="button"
          onClick={goHome}
          ...
          style={{
            ...
            fontSize: 20, fontWeight: 500, fontFamily: '"Cormorant Garamond", Georgia, serif',
            color: 'var(--warm-ink)', letterSpacing: '-0.02em', cursor: 'pointer',
        ...
        <ul className="hd-nav-desktop" style={{ display: 'flex', gap: 28, listStyle: 'none', margin: 0, padding: 0 }}>
          {[
            ...
            <li key={item.label}>
              <span
                onClick={item.action}
                style={{ fontSize: 14, fontWeight: 400, color: 'var(--warm-muted)', letterSpacing: -0.1, cursor: 'pointer', transition: 'color 0.15s' }}
                onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--warm-ink)')}
        ...
        <div className="hd-nav-right" style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 13, color: 'var(--warm-muted)',
            ...
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--warm-primary)'; e.currentTarget.style.color = 'var(--warm-body)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--warm-hairline)'; e.currentTarget.style.color = 'var(--warm-muted)'; }}
          >
          ...
          <span
            ...
            style={{ fontSize: 14, color: 'var(--warm-primary)', letterSpacing: -0.1, cursor: 'pointer', whiteSpace: 'nowrap' }}
        ...
        >
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
            <line x1="3" y1="6" x2="19" y2="6" stroke="var(--warm-ink)" strokeWidth="1.6" strokeLinecap="round"/>
            <line x1="3" y1="11" x2="19" y2="11" stroke="var(--warm-ink)" strokeWidth="1.6" strokeLinecap="round"/>
            <line x1="3" y1="16" x2="19" y2="16" stroke="var(--warm-ink)" strokeWidth="1.6" strokeLinecap="round"/>
          </svg>
        </button>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px 16px', borderBottom: '1px solid var(--warm-hairline)' }}>
                <span style={{ fontFamily: '"Cormorant Garamond", Georgia, serif', fontSize: 20, fontWeight: 500, color: 'var(--warm-ink)', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button
                  onClick={() => setMobileOpen(false)}
                  aria-label="关闭菜单"
                  style={{ width: 36, height: 36, background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 22, color: 'var(--warm-muted)', lineHeight: 1 }}
                >
                  ×
                </button>
              </div>
              <nav style={{ display: 'flex', flexDirection: 'column', padding: '8px 0' }}>
                {mobileNavItems.map((item, i) => (
                  <motion.span
                    ...
                    style={{
                      padding: '14px 24px', fontSize: 16, color: 'var(--warm-ink)',
                      cursor: 'pointer', borderBottom: '1px solid var(--warm-surface)',
              ...
              <button
                ...
                style={{
                  ...
                  padding: '0 14px', fontSize: 14, color: 'var(--warm-muted)',
                  background: 'var(--warm-surface)', border: '1px solid var(--warm-hairline)',
